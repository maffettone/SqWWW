program main
  ...
  character(len=100):: cell_in                              !Cif for cell if structure is pdb
  ...
  if (struct_in(len_trim(struct_in)-3:len_trim(struct_in))=='.pdb') then
     call read_pdb(struct_in,cell_in)
     is_fract = .false.
  else
  ...
  if(is_fract) call gen_adj()   !is_fract at this point means only a cif was read in
  ...
  WWW_Loop: do i=1,out_moves
     ...
     if(out_moves>=100 .and. mod(i,out_moves/100) == 0) then
...
15   select case(trim(thermostat))
     ...
     else if (out_moves>= 500 .and. mod(i,out_moves/5000) == 0) then
        filename = trim(out_dir)//'time_data.csv'
        call write_time(filename,i)
     end if
...
Contains
  ...
  subroutine read_pdb(pdbname,cifname)
    ! Reads a pdb for coordinates and bonding, and cif for cell parameters
    ...
    open(newunit=unit, file=trim(pdbname), status='old',IOSTAT = info)
    if(info /= 0 ) then
       write(*,*)'Failure in pdb reader. File not found'
       write(*,*) pdbname
       stop
    end if
    ! Getting atom coordinates and symbolsx
    target='ATOM'
    do while(trim(line)/=trim(target))
       read(unit,*) line
    end do
    backspace(unit)
    do i=1,size(sym)
       read(unit,fmt)line,idx,line,sym(i),line,line,line,line,j,line,line,coords(:,i),dump,dump,line,line,line
    end do
    ! Generating adjacency from CONECT
    adj=0
    target='CONECT'
    do while(trim(line)/=trim(target))
       read(unit,*) line
    ...
    info=0
    do i=1,size(sym)
       read(unit,"(A6,I5)",IOSTAT=info,ADVANCE='NO') line,idx
       if(info/=0)exit
       do
          read(unit,"(I5)",IOSTAT=info,ADVANCE='NO')j
          if (info==-2) exit
          adj(j,idx)=1
       end do
    end do
    do i=1,size(adj,1)
       do j=1,size(adj,2)
          if(adj(j,i)>0) then
             if(adj(i,j)<adj(j,i))adj(i,j)=adj(j,i)
    ...
    ! Getting cif info
    open(newunit=unit, file=trim(cifname), status='old',IOSTAT = info, IOMSG=line)
    if(info /= 0 ) then
       write(*,*)'Failure in pdb reader. File not found'
       write(*,*) cifname
       write(*,*) info, line
  ...
  subroutine get_input(filename)
    ...
    do
       ...
       if (line(1:6) == 'struct') then
          ...
          cycle
       else if (line(1:4) == 'cell') then
          read(line(5:120),'(A116)',iostat=info) cell_in
          if(info/=0) then
             write(*,*)'Bad filename for input cell'
             stop
          end if
          cell_in = trim(adjustl(cell_in))
          cycle
